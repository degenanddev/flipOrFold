import { getAddress, verifyMessage } from 'npm:ethers@6'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!message.includes('recover game account')) {
      return json({ error: 'Invalid recovery message' }, 400)
    }

    const wallet = getAddress(wallet_address).toLowerCase()
    const recovered = verifyMessage(message, signature)

    if (getAddress(recovered).toLowerCase() !== wallet) {
    ...
    if (playerErr || !player?.username) {
      return json({ error: 'No linked account for this wallet' }, 400)
    }
    ...
    const { data, error } = await supabase.rpc('finalize_wallet_recovery', {
      p_new_device_id: device_id,
